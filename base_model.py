...
class BaseModel(nn.Module, PyTorchModelHubMixin, repo_url="https://github.com/JusperLee/Apollo", pipeline_tag="audio-to-audio"):
    ...
    def from_pretrain(pretrained_model_conf_or_path, *args, **kwargs):
        ...
        # Some community checkpoints (e.g. Baicai1145 Vocal MSST) don't store
        # `model_name` at the top level. Look for common fallbacks before
        # defaulting to Apollo.
        model_name = conf.get("model_name")
        if model_name is None:
            hyper_params = conf.get("hyper_parameters") or {}
            model_name = hyper_params.get("model_name")
        if model_name is None:
            # Last resort: the project only exposes `Apollo`, so keep backward
            # compatibility by defaulting to it when the checkpoint omits the
            # field.
            model_name = "Apollo"
